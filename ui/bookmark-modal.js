...
export function renderBmList() {
  ...
  if (currentUser.role === 'admin') {
    ...
    // Member & Guest: 공개 목록(읽기) + 내 북마크(편집 가능)
    renderBmSection('공개', 'public', false, bookmarks);
...
function renderBmSection(label, vis, ownerOnly, bookmarks) {
  ...
  items.forEach(bm => {
    ...
    // 편집/삭제 버튼: Admin이거나 자신의 항목일 때만 표시
    const isAdmin = currentUser.role === 'admin';
    const isOwner = bm.owner === currentUser.email;
    ...
    // 드래그앤드롭: Admin이거나 자신의 항목일 때만 활성화
    if (isAdmin || isOwner) {
